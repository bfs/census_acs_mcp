 ...
 */
export async function getAreaSummary(location, tables) {
    ...
    // Get land area
    const areaResult = await query(`SELECT aland FROM json_db.tiger_geometries WHERE geo_id = ?`, [geo_id]);
 ...
 */
export async function compareAreas(location_a, location_b, metric_ids, table_ids) {
    ...
    // Build metric filter
    let metricFilter = '';
    const params = [resolvedA.geo_id, resolvedB.geo_id];
    ...
    // Get comparison data
    const comparisonResults = await query(`SELECT 
       a.uid,
       m.label,
     ...
     WHERE a.geo_id = ?
       ...
       ${metricFilter}
     LIMIT 100`, params);
    const comparisons = comparisonResults.map((row) => ({
        metric_id: row.uid,
        label: row.label,
        value_a: row.value_a,
        value_b: row.value_b,
