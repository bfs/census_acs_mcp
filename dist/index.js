    ...
    try {
        ...
        switch (name) {
            ...
            case 'compare_areas': {
                ...
                const result = await compareAreas(params.location_a, params.location_b, params.metric_ids, params.table_ids);
                if (!result) {
            ...
            case 'get_interesting_facts': {
                ...
                const result = await getInterestingFacts(params.location, params.threshold, params.limit, params.category);
                if (!result) {
                    return { content: [{ type: 'text', text: `Could not resolve location: ${params.location}` }] };
                }
                return { content: [{ type: 'text', text: JSON.stringify(result, null, 2) }] };
