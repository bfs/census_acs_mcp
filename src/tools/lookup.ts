...
export async function searchLocations(
  ...
  const results = await query<{ geo_id: string; name: string }>(
    `SELECT geo_id, name 
  ...
  // Get total count
  const countParams: unknown[] = [trimmed];
  if (summaryLevel) {
    const levelCode = SUMMARY_LEVEL_NAMES[summaryLevel] || summaryLevel;
...
export async function listGeographies(
  ...
  parentGeoId?: string,
  limit: number = 100
): Promise<ListGeographiesResult> {
  // Accept either code ("040") or name ("state")
  const levelCode = SUMMARY_LEVEL_NAMES[summaryLevel] || summaryLevel;
  const levelName = SUMMARY_LEVELS[levelCode] || summaryLevel;

  const params: unknown[] = [levelCode + '%'];
  let parentFilter = '';
  ...
  // Get total count
  const countParams: unknown[] = [levelCode + '%'];
  if (parentGeoId) {
    const parentStateFips = parentGeoId.slice(-2);
...
export async function resolveLocation(
  ...
  if (/^\d/.test(trimmed) && trimmed.includes('US')) {
    const result = await query<{ geo_id: string; name: string }>(
      `SELECT geo_id, name FROM json_db.tiger_geometries WHERE geo_id = ?`,
      [trimmed]
    );
    if (result.length > 0) {
  ...
  const nameResults = await query<{ geo_id: string; name: string }>(
    `SELECT geo_id, name 
     FROM json_db.tiger_geometries 
     WHERE name ILIKE '%' || ? || '%'
     ${levelFilter}
 ...
 */
export function getSummaryLevel(geoId: string): string {
...
export async function lookupLocation(
  ...
  const geoResults = await query<{ geo_id: string; name: string }>(
    `SELECT geo_id, name
     FROM json_db.tiger_geometries
  ...
  let tableFilter = '';
  const params: unknown[] = [geo.geo_id];
  ...
  const data: TableData[] = dataResults.map((row) => {
