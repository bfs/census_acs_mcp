...
import type { RankAreasResult, RankResult, ListPopulationGroupsResult } from '../types.js';

// Known population group codes and their names
// These are derived from ACS table naming conventions
const POPULATION_GROUP_NAMES: Record<string, string> = {
 ...
 */
export async function listPopulationGroups(): Promise<ListPopulationGroupsResult> {
  // Query distinct group codes from state-level data
  const results = await query<{ group_code: string; record_count: number }>(
    `SELECT 
...
export interface RankAreasParams {
  metric_id: string | string[];  // Single metric or array to sum
  denominator_id?: string | string[];  // Single metric or array to sum for rate calculation
  order?: 'desc' | 'asc';
  percentile_min?: number;
  percentile_max?: number;
  summary_level?: string;
  state_fips?: string;
  population_group?: string;  // Filter by population group code (e.g., "0000" for total)
 ...
 */
export async function rankAreasByMetric(params: RankAreasParams): Promise<RankAreasResult> {
  const {
    metric_id,
    denominator_id,
    order = 'desc',
    ...
    summary_level,
    state_fips,
    population_group = '0000',  // Default to total population
  ...
  const buildPopulationGroupFilter = (alias: string): string => {
    if (!population_group) return '';
    return `AND SUBSTRING(${alias}.geo_id, 4, 4) = '${population_group}'`;
  };

  // Get metric label for response
  const firstMetricId = metricIds[0];
  const metricInfo = await query<{ title: string; label: string }>(
    `SELECT title, label FROM pct_db.table_metadata WHERE unique_id = ?`,
    [firstMetricId]
  );
  ...
  const buildInClause = (ids: string[]): { clause: string; params: string[] } => {
  ...
  if (denominatorIds) {
    ...
    const countResult = await query<{ cnt: number }>(countSql, countParams);
    ...
    const queryParams: unknown[] = [...metricIn.params, limit];
    ...
    const countResult = await query<{ cnt: number }>(countSql, metricIn.params);
    totalMatches = countResult[0]?.cnt || 0;
    ...
    if (summary_level) queryParams.push(summary_level);
    if (state_fips) queryParams.push(state_fips);
    ...
    if (summary_level) countParams.push(summary_level);
    if (state_fips) countParams.push(state_fips);
